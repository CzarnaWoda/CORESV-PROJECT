...
public class WhoisCommand extends Command implements Colors {
	...
	public boolean onExecute(CommandSender sender, String[] args) {
		...
		Util.sendMsg(sender, Util.replaceString(SpecialSigns + "  * " + MainColor + "Zbanowany IP: " + ImportantColor + ((bip != null) ? (((bip.getExpireTime() == 0L) ? "permamentnie" : "tymczasowo (wygasa " + Util.getDate(bip.getExpireTime()) + ")") + MainColor + ", powod: " + ImportantColor + bip.getReason() + MainColor + ", przez: " + ImportantColor + bip.getAdmin()) : ChatColor.DARK_RED + "%X%")));
		Util.sendMsg(sender, Util.replaceString(SpecialSigns + ">> " + MainColor + "Informacje o graczu " + SpecialSigns + "* " + WarningColor + u.getLastName() + SpecialSigns + " <<"));
		return false;
	}
