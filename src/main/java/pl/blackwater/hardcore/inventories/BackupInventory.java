...
public class BackupInventory implements Colors {
    public static InventoryGUI openInventory(){
        InventoryGUI inv = new InventoryGUI(Core.getInstance(), Util.fixColor(Util.replaceString(SpecialSigns + "->> " + MainColor + "BACKUP " + MainColor + "MENU " + SpecialSigns + "<<-")), 3);
        ...
        inv.setItem(13, globalsave.build(), (player, inventory, i, itemStack) -> {
            ...
            for(Player online : Bukkit.getOnlinePlayers()){
                ...
                Core.getBackupManager().createBackup(online, BackupType.GLOBAL, player.getName());
    ...
    public static void openPlayerMenu(Player player, Player other){
        InventoryGUI inv = new InventoryGUI(Core.getInstance(), Util.fixColor(Util.replaceString("->> " + MainColor + "BACKUPS " + SpecialSigns + " ->> " + ImportantColor + BOLD  + other.getName().toUpperCase())), 6);
    ...
    public static void openBackup(Player player, Backup backup){
        InventoryGUI inv = new InventoryGUI(Core.getInstance(), Util.fixColor(Util.replaceString("->> " + MainColor + "BACKUPS " + SpecialSigns + " ->> " + ImportantColor + BOLD  + backup.getBackupUUID().toString())), 6);
