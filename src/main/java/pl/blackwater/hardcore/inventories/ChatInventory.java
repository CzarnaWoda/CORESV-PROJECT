...
public class ChatInventory implements Colors
{
	...
	{
		final InventoryGUI inv = new InventoryGUI(Core.getInstance(), Util.fixColor(Util.replaceString(SpecialSigns + "* " + ImportantColor + "ChatManager" + SpecialSigns + " *")), 1);
		...
		inv.setItem(2, toggle.build(), (p12, arg1, arg2, arg3) -> {
			ChatManager manager = Core.getChatManager();
			manager.toggleChat(p12.getName());
			p12.closeInventory();
		...
		inv.setItem(8, togglevip.build(), (p13, arg1, arg2, arg3) -> {
			ChatManager manager = Core.getChatManager();
			manager.toggleVipChat(p13.getName());
			p13.closeInventory();
		...
		final ItemBuilder killstowrite = new ItemBuilder(Material.DIAMOND_SWORD).setTitle(Util.fixColor(Util.replaceString(SpecialSigns + ">> " + MainColor + "Ustaw limit " + ImportantColor + "punktow " + MainColor + "do pisania na chacie")))
				.addLore(Util.fixColor(Util.replaceString(SpecialSigns + "  * " + MainColor + "Aktualny limit: " + ImportantColor + CoreConfig.CHATMANAGER_POINTS)));
	...
	private static void openSlowModeMenu(Player player) {
		final InventoryGUI inv = new InventoryGUI(Core.getInstance(), Util.fixColor(Util.replaceString(SpecialSigns + "* " + ImportantColor + "Ustaw limit czasu" + SpecialSigns + " *")), 1);
